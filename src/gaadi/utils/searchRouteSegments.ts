import { type IRoute } from "../types/stop.types";
...
function getRouteById(routeId: string): IRoute | undefined {
  return (routes as any[]).find((r) => r.id === routeId);
}
...
function buildRouteSegmentsFromPath(
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    if (isTransfer) {
      ...
      currentStops = [currentStops[currentStops.length - 1]];
    }
  ...
  while (i < segments.length) {
    if (i < segments.length - 1) {
      ...
      if (shouldAttemptMerge) {
        ...
        const mergedRoute = (routes as any[]).find((r) => {
...
async function searchRouteSegments(
  ...
  try {
    ...
    const directRoute = (routes as any[]).find((route) => {
    ...
    while (queue.length > 0) {
      ...
      if (stop === toStopId) {
        ...
        for (let i = 0; i < segments.length - 1; i++) {
          const endStop = segments[i].stops[segments[i].stops.length - 1];
